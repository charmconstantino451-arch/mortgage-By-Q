    ...
    >
      <div
        className={`mx-auto max-w-7xl px-6 flex items-center justify-between transition-all duration-500 ${
          scrolled
            ? 'bg-black/80 backdrop-blur-xl border border-white/[0.06] rounded-full py-3 px-6'
            : 'bg-transparent'
        }`}
      >
        {/* Logo */}
        <a href="#hero" className="flex items-center gap-3 group flex-shrink-0">
          <div className="w-7 h-7 bg-gold flex items-center justify-center font-serif text-black text-sm font-bold leading-none select-none flex-shrink-0">
            Q
          </div>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="nav-link text-[11px] uppercase tracking-[0.12em] text-white/60 hover:text-gold font-sans font-medium"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="btn-gold text-[11px] uppercase tracking-wider py-2.5 px-5 flex items-center gap-1.5 group animate-pulse-gold"
          >
            Get My Free Quote
